...
if (isset($_REQUEST['entrar'])) { //Si le ha dado al boton de enviar valido los datos
    ...
    try{
        ...
        $consulta = "SELECT T01_Password FROM T01_Usuario WHERE T01_CodUsuario=:CodUsuario"; //Creo la consulta y le paso el usuario a la consulta
        ...
        if($resultadoConsulta->rowCount()>0){ //Si la consulta tiene algun registro
            ...
            if($oUsuario->T01_Password != $passwordEncriptada){ //Compruebo si la password es correcta
                $aErrores['CodUsuario'] = "Error en la autentificacion."; //Si no es correcta, almaceno el error en el array de errores
                $aErrores['Password'] = "Error en la autentificacion."; //Si no es correcta, almaceno el error en el array de errores
            }
        }else{ //Si no he recibido ningun registro no existe el usuario en la DB
            $aErrores['CodUsuario'] = "Error en la autentificacion."; //Si no es correcto, almaceno el error en el array de errores
            $aErrores['Password'] = "Error en la autentificacion."; //Si no es correcto, almaceno el error en el array de errores
        }
...
if($entradaOK){ //Si la entrada es correcta
    try{
        ...
        $consulta = "SELECT T01_NumConexiones, T01_FechaHoraUltimaConexion FROM T01_Usuario WHERE T01_CodUsuario=:CodUsuario"; //Creo la consulta y le paso el usuario a la consulta
        $resultadoConsulta2=$DB207DWESProyectoTema5->prepare($consulta); // Preparo la consulta antes de ejecutarla
        $aParametros2 = [
            ":CodUsuario" => $_REQUEST['CodUsuario']
        ];
        $resultadoConsulta2->execute($aParametros2);//Ejecuto la consulta con el array de parametros 
        
        $oRegistro = $resultadoConsulta2->fetchObject(); //Obtengo el primer registro de la consulta y lo guardo en una variable
        
        $numeroConexiones = $oRegistro->T01_NumConexiones; //Guardo el numero de conexiones que tiene la base de datos en una variable
        $ultimaConexion = $oRegistro->T01_FechaHoraUltimaConexion; //Guardo la fechahora de la ultima conexion que tiene la base de datos en una variable
        ...
        $resultadoConsultaUpdate = $DB207DWESProyectoTema5->prepare($consulta); // Preparo la consulta antes de ejecutarla
        $aParametros3 = [ //Array de parametros para el update
            ...
            ":FechaHoraUltimaConexion" => time(), //Asigno hora local actual como marca temporal
        ...
        session_start(); //Creo una nueva sesion
        $_SESSION['usuarioLogin'] = $_REQUEST['CodUsuario']; //Almaceno el usuario en $_SESSION
        $_SESSION['fechaLogin'] = $ultimaConexion; //Almaceno la ultima conexion en $SESSION
